...
public class NexgoReadCard extends AppCompatActivity {
    ...
    private void showInputPin(boolean isOnlinPin, int leftTimes) {
        ...
        // // There is no key injected; cannot continue - show some error to user and
        // break
        // // out
        // Log.e("Nexgo", "startInputPin() : cannot continue, No key is injected!");
        // emvHandler2.emvProcessCancel(); // Stop the EMV process, cannot proceed to
        // enter PIN without the injected
        // // key
        // return;
        // }
        ...
        mContext.runOnUiThread(() -> {
                    ...
                    if (retCode == SdkResult.Success || retCode == SdkResult.PinPad_No_Pin_Input
                        ...
                        if (data != null) {
                            if (isOnlinPin) {
                                ...
                                pinBlockArray = ByteUtils.byteArray2HexString(data).toUpperCase(); // Set the
                                                                                                   // pinBlockArray
                                                                                                   // (String) to the
                                                                                                   // return value
                                                                                                   // 'data' (PIN
                                                                                                   // output) for
                                                                                                   // sending to host
                                pinKsn = ByteUtils.byteArray2HexString(pinPad.dukptCurrentKsn(0)).toUpperCase(); // Save
                                                                                                                 // the
                                                                                                                 // pinKsn
                                                                                                                 // in
                                                                                                                 // case
            ...
            if (isOnlinPin) {
                ...
                pinPad.inputOnlinePin(pinLen, 60, cardNo.getBytes(), INJECTED_PIN_SLOT,
