...
public class EmpressaPosPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    if (call.method.equals("searchCard")) {
      searchCard(result);
    } else {
  ...
  private void searchCard( @NonNull Result result) {
    try {
  ...
  public void startProcess(@NonNull Result result) {
                ...
                public void onConfirm(final byte[] data, boolean isNonePin) {
                  ...
                  if (!isNonePin) {
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                      @Override
                      public void run() {
                        Toast.makeText(mContext, "Password:" + HexUtil.toString(data), Toast.LENGTH_LONG).show();
                      }
                    });
                  }
      ...
      public void processResult(int ret) {
        ...
        if (ret == 0 || ret == 1 || ret == 2) {
          ...
          if (tlvData != null) {
            ...
            //updateResult("[getTLVData]" + result);
  ...
  private void readcard( @NonNull Result result) {
  ...
  private void updateICCardData( @NonNull Result methodResult) {
